...
summary = {
    ...
    "popularity": pop_df["popularity"],
    "release_year": year_df["release_year"].value_counts().sort_index(),
    "hours": time_df["hour"].value_counts().sort_index(),
    "weekdays": time_df["weekday"].value_counts().reindex(
...
if not summary["genre_counts"].empty and "genre" in summary["genre_counts"].columns and "count" in summary["genre_counts"].columns:
    ...
    plt.title("Top Genres Frequency")
...
plt.xlabel("Day"); plt.ylabel("Count"); plt.tight_layout()
plt.show()
